...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!bruger) {
            return res.status(401).json({ message: "Bruger findes ikke", login_godkendt: false })
        }
        ...
        bruger.comparePassword(password, function (err, isMatch) {
            ...
            if (isMatch) {
                ...
                res.status(401).clearCookie(process.env.SESSION_NAME).json({ message: "Bruger findes ikke", login_godkendt: false });
            }
        });
...
router.get('/logout', async (req, res) => {
    ...
    req.session.destroy(err => {
        ...
        res.clearCookie(process.env.SESSION_NAME).json({ message: 'Bruger logget ud - cookie slettet' });
